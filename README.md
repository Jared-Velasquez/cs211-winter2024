...
# (Previous Task D README — appended)
...
## Evaluation and Analysis (Student D)

Once Student C has saved per-candidate predictions to disk, Student D's tools score them and analyze the proxy-vs-accuracy correlation:

- [`evaluate.py`](./evaluate.py): score one set of saved predictions against ground truth
- [`aggregate_results.py`](./aggregate_results.py): score every candidate listed in a partition manifest, producing one `partition_results.json` per task
- [`analyze_partitions.py`](./analyze_partitions.py): per-task and cross-task Spearman correlation between proxy metrics and headline accuracy drop, plus partition-strategy comparison
- [`plot_partitions.py`](./plot_partitions.py): Pareto plots (latency vs. accuracy), proxy-vs-drop scatter plots, and a flat summary CSV

See [`docs/student_d_workflow.md`](./docs/student_d_workflow.md) for the end-to-end recipe.
